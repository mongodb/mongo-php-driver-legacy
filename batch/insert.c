...
PHP_METHOD(MongoInsertBatch, __construct)
{
	...
	php_mongo_api_batch_ctor(intern, zcollection, MONGODB_API_COMMAND_INSERT, write_options TSRMLS_CC);
}
/* }}} */
...
static zend_function_entry MongoInsertBatch_methods[] = {
	PHP_ME(MongoInsertBatch, __construct, arginfo___construct, ZEND_ACC_PUBLIC)
	PHP_FE_END
};

void mongo_init_MongoInsertBatch(TSRMLS_D)
{
	zend_class_entry insert_batch;
	INIT_CLASS_ENTRY(insert_batch, "MongoInsertBatch", MongoInsertBatch_methods);

	insert_batch.create_object = php_mongo_write_batch_object_new;
