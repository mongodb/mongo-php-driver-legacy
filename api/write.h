...
typedef struct {
	php_mongodb_write_types type;
	union {
		HashTable *insert;
		php_mongodb_write_update_args *update;
		php_mongodb_write_delete_args *delete;
	} write;
} php_mongodb_write_item;

typedef struct _php_mongodb_batch {
	struct _php_mongodb_batch *first;
	struct _php_mongodb_batch *next;
	mongo_buffer buffer;
	int request_id;
	int container_pos;
	int batch_pos;
