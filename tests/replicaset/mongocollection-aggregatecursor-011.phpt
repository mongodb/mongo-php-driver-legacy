--TEST--
MongoCollection::aggregateCursor() with $out should force primary
--INI--
error_reporting=-1
--SKIPIF--
...
foreach ($readPrefs as $readPref) {
    ...
    /* We'll check that the collection read pref is not disturbed, even though
     * aggregateCursor() never overwrites it, since it forces the primary read
     * pref on the command cursor directly.
     */
    $rp = $coll->getReadPreference();
    printf("Collection read preference: %s\n", $rp['type']);
