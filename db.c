...
/* {{{ proto MongoCommandCursor MongoDB::commandCursor(array command)
...
PHP_METHOD(MongoDB, commandCursor)
{
	...
	mongo_command_cursor_init(cmd_cursor, Z_STRVAL_P(db->name), db->link, command TSRMLS_CC);

	/* Add read preferences to cursor, overriding the one set on the link */
	mongo_read_preference_replace(&db->read_pref, &cmd_cursor->read_pref);
}
/* }}} */
